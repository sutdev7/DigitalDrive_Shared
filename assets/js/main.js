...
$(function () {
	...
	$("#postduedate").datepicker({
	  dateFormat: 'dd-mm-yy'
	});
	
	$('body').on('focus',".duedatemilestone", function(){
		$(this).datepicker({
			dateFormat: 'mm-dd-yy'
		});
	});
...
$(function() {
	$('body').on('change', '.picupload', function(event) {
		 ...
		 $('#media-list').append('<li class="myupload"><span><i class="fa fa-plus" aria-hidden="true"></i><input type="file" name="fldTaskDocuments[]"  click-type="type2" id="picupload" class="picupload" multiple></span></li>');
	});	
...
// custom code
//	$('#positive_coin').on('click',function(){
//		alert('Working on');
//	});
...
$(document).on('click','.addcategory',function(){
		...
		$.ajax({
			url:'/hwhavenew/addSkill',
			type:'POST',
			dataType: "json",
			data:{'title':title},
			success:function(response){
				//alert(response.lastinsertid);
				$('.form-control-chosen').append($("<option/>", {
					value:response.lastinsertid,
					text: searchtext
				}));
				$(".form-control-chosen option[value='"+response.lastinsertid+"']").prop("selected", true);
