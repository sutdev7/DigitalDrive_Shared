...
class Task extends CI_Controller {
	...
	public function update_microkey($taskID = null){
		$data = array();
        ...
        $submitData = $this->input->post();
        if($this->form_validation->run() == false){
	        //$content = $this->ltask->update_microkey($taskID, $submitData);
	        $content = $this->ltask->update_microkey_client($taskID, $this->session->all_userdata());
	        $data = array(
		        'content' => $content,
		        'title' => display('Edit Task Step - 3 :: Hire-n-Work'),
	        );
        	...
        	$this->ltask->update_microkey_client($taskID, $this->session->all_userdata());
	    }
	}
