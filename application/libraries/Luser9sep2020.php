...
exit; */
	...
	public function public_profile_page(){
        ...
        if(!empty($userData)){
			$positivecoin=0;			
            if($userLoginData->total_coins>=0){ $positivecoin='+ '.$userLoginData->total_coins;}else{ $positivecoin=$userLoginData->total_coins;}
            $userInfo = array(
		...
		if(count($reviews_data)>0 ){
			...
			foreach($reviews_data as $rv){
                ...
                $rpositivecoin=0;			
                if($rv->total_coins>=0){ $rpositivecoin='+ '.$rv->total_coins; }else{ $rpositivecoin =$rv->total_coins; }
				$reviews[]=array(
