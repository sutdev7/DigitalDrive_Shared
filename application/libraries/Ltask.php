...
//                        dd($data);
  ...
  $postData['uploadFiles'] = array();
	...
	public function browse_task_page($pageIndex = 0) {
      ...
      if($page/$per_page >= 4) redirect('sign-up-as/1', 'refresh');
	   ...
	   if(isset($_POST['searchCriteria'])){
        ...
        foreach($jobs as $job) {
        	...
        	$arrJobs[] = array('task_id' => $job->task_id, 'user_task_id' => $job->user_task_id, 'task_name' => $job->task_name, 'task_details' => $job->task_details, 'task_total_budget' => $job->task_total_budget, 'task_created_by' => $userData['basic_info']->name, 'user_country' => $userData['basic_info']->country,'total_coins' => $userData['basic_info']->total_coins, 'user_state' => $userData['basic_info']->state, 'last_login' => isset($userData['basic_info']->last_login)?$userData['basic_info']->last_login:'', 'user_city' => $userData['basic_info']->city, 'user_is_online' => $user_is_login, 'task_skill_requirements' => $arrTaskSkills, 'task_post_duration' => $minutes, 'task_offers' => $task_offers_count, 'user_image' => $user_profile_image,'total_project_completed'=>$allanalytics['yearly_projects'],'total_money_spent'=>$allanalytics['yearly_income'],'total_hired_freelancer'=> $hired_freelancer);        	
...
/*		$micro_client_jobs = $CI->Tasks->get_all_micro_client_task_lists($micro_key_offset,$micro_key_limit,$SearchKeyWord);	
    ...
    public function task_details_page($taskID = null, $userInfo = null) {
        ...
        $offer_send = $CI->Tasks->get_proposal($taskID);
        $proposals = $CI->Tasks->get_freelancers_proposal($taskID);
        foreach($offer_send as $offer) {
            $user_details = $CI->Users->get_user_profile_info_by_id($offer->user_id);
            $profile_img = $user_details['basic_info']->profile_image;
            if ($profile_img != NULL) {
...
public function sent_offer_page_new($userInfo = null, $type) {
    ...
    public function received_offers_page($userInfo = null, $user_task_id = null) {
        ...
        foreach($jobs as $job) {
            ...
            $posting_date = date('d/m/Y, h iA',strtotime($job->doc));
            ...
            $arrJobs[] = array('user_task_id' => $user_task_id, 'terms_amount_max' => $job->terms_amount_max, 'terms_amount_min' => $job->terms_amount_min, 'remarks' => $job->cover_letter, 'task_id' => $job->task_id, 'proposal_id' => $job->proposal_id, 'user_profile_image' => $user_profile_image, 'user_is_login' => $user_is_login, 'task_post_duration' => $minutes,
                'name' => $userData->name,
