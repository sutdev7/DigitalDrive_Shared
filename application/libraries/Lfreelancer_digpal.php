...
public function analytics_details(){		
		...
		// Pagination Configuration
		$config['base_url'] = base_url().'see-all-projects';
		$config['full_tag_open'] = '<ul class="pagination" style="margin-top:20px;">';
		$config['full_tag_close'] = '</ul>';
		$config['first_link'] = true;
		$config['first_tag_open'] = '<li class="previous">';
		...
		//$config['total_rows'] = $CI->Microkeys->get_projects_count($searchValue);
		$config['total_rows'] = 5;
		$config['per_page'] = $rowperpage;
		//echo $CI->session->userdata('user_id');
		...
		//$data['analytics'] = $CI->Tasks->project_details1($CI->session->userdata('user_id'), 100);
		$data['analytics'] = $CI->Tasks->project_details1($CI->session->userdata('user_id'), $searchValue,$rowperpage,$rowno);
