                ...
                <div class="col-lg-8">
                    <div class="task_Left">
                        ...
                        <div class="task_Left_Div">
                            <h3>{task_title}</h3>
                            <h2 class="black_color">Posted {created_on} <!--;<br> Due Date {task_info}{task_due_date}--></h2>

                            <h4 class="black_color">{country}, {continent}</h4>
                            <p>{description}</p>

                            <div class="task_info pt-5 border-bottom-0">
                                <span>  <h5> Hours to be determined</h5>
                                    <em><i class="fa fa-clock-o"></i> {duration_type}</em></span>
                                <span>  <h5> Project length</h5>
                                    <em><i class="fa fa-calendar"></i> {duration} </em></span>
                                <span>
                                    <h5>Budget</h5>
                                    <em><i class="fa fa-dollar" aria-hidden="true"></i> {budget}</em> 
                                </span>
                                <span>
                                    <h5>Proposal</h5>
...
<!--                                <h2 class="Atta mt-0">Project Type </h2>
                               ...
                               <!-- <h2 class="Atta mt-0">Keywords</h2>
                                ...
                                <div>
                                    <span class="w-100">{skill_name}<a href="#">{name}</a>{/skill_name}</span>
                                </div>
...
<!--                                        <p class="mb-1">Proposal <b> {proposal_count}</b></p>
                                        ...
                                        <p class="mb-1">Hired <b> {task_freelancer_hire}</b></p>
                ...
                <div class="col-lg-4">
                    <div class="task_Right_Div">
                        ...
                        if ($hire_me == true) {
                            ...
                            ?>
                            <form action="<?= base_url() . 'add-microkey-proposal' ?>" method="post">
                                <input type="hidden" value="{client_id}" name="microkey_user_id" id="microkey_user_id">
                                <input type="hidden" value="{microkey_job_id}" name="microkey_id" id="microkey_id">
                                <input type="submit" value="Submit Proposal" class="eyeBtn">
                                <a href="#" onclick="return false;" class="{savetextclass}" id="txtShow" >{savetext}</a>
                            </form>
                            <?php
                        }
                        ?>
                        <div class="offerDiv">
                            <ul>
                                <li>
                                    <h3><a href="#" onclick="return false;" class="{inappropriateclass}" id="txtflag" ><i class="fa fa-flag mr-2"></i>{inappropriatetext}</a></h3>
                            ...
                            <h4>About Client : {client_name}</h4>
                            <ul>
                                <li>
                            ...
                            <ul>
                                <li>
                                    <h3><a href="<?php echo base_url() . 'microkey-job-details/' . $this->uri->segment(2); ?>" id="copylink" onclick="return false;"><?php echo base_url() . 'microkey-job-details/' . $this->uri->segment(2); ?></a></h3>
...
<script> 
    $(document).ready(function() {
        $('.saveBtn').on('click',function(){
            $.ajax({
                type: "POST",
                url: "<?= base_url() . 'save-microkey-job' ?>",
                data: {microkeyId : $('#microkey_id').val() },
                dataType: "text",
                success: function(resultData){ 
                    $('txtShow').removeClass('saveBtn');
        ...
        $('.flagBtn').on('click',function(){
            $.ajax({
                type: "POST",
                url: "<?= base_url() . 'save-inappropriate-microkey' ?>",
                data: {microkeyId : $('#microkey_id').val() },
                dataType: "text",
                success: function(resultData){ 
                    $('#txtflag').removeClass('flagBtn');
