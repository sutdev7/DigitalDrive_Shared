...
class Freelancer extends CI_Controller {
	...
	function __construct() {
		...
		$this->load->library('Ltask');
                if($this->uri->segment(1) != 'job-details-front')
                {
...
//	        $content = $this->lmicrokey->post_microkey_client($submitData);
//	        $data = array(
//		        'content' => $content,
...
//		    $this->template->full_customer_html_view($data);
        	...
        	if(!empty($submitData)) {
                    $table_data['task'] = array('task_name' => $submitData['title'], 'task_details' => $submitData['description'], 'task_due_date' => "", 'task_origin_location' => $submitData['continent'], 'task_origin_country' => $submitData['fldSelCountry'], 'task_total_budget' => $submitData['budget'], 'task_status' => 1,'task_keywords' =>"",'task_duration_type' =>$submitData['duration_type'],'task_duration' =>$submitData['duration']); 			
                    ...
                    if (!empty($_FILES) && is_array($_FILES)) {
                        if (!empty($_FILES['image']['name'])) {
                            ...
                            $targetPath = "./uploads/user/project_documents/" . $filename;
                            if (move_uploaded_file($sourcePath, $targetPath)) {
                                $table_data['task_attachments'][] = $filename;
                            }
                        }
                    }
                    $result = $this->Tasks->add_new_task($this->session->all_userdata()['user_id'], $table_data); 
                    if($result) {
                        $this->session->set_flashdata('msg', '<div class="alert alert-success text-center">New Task added successfully.</div>');
                    }else {
                        $this->session->set_flashdata('msg', '<div class="alert alert-danger text-center">Unable to add new task.</div>');
                          }	                           	
                }
	...
	public function freelancer_job_details(){
        ...
        public function freelancer_job_details_front(){
		$task_id = $this->uri->segment(2);
		$content = $this->lfreelancer->freelancer_job_details($task_id, 1);
		$data = array(
                        'content' => $content,
                        'title' => display('Freelancer Job Details :: Hire-n-Work'),
                );
